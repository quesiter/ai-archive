...
5. 密码、密钥、私钥和登录凭据不以原文入库；其他敏感数据尽量留在本地，发往模型前脱敏。
...
| `apps/web` | 管理后台 UI，包括仪表盘、会话、项目、报告、导入、设备、日志、二级设置、系统状态和独立更新记录。 |
...
所有调用 `completeStructured` 的分类、知识、周报和月报共用进程级 AI 请求节流器。默认最小调用起始间隔为 82 秒；首个请求立即执行，后续并发请求按同一时间轴排队。该间隔来自 244 次调用耗尽五小时额度的线上实测，使同等调用量约 5.5 小时完成，并可在设置页调整或停用。

Worker 通过 `nightly-ai-maintenance` 队列编排每日夜间维护。Asia/Shanghai 22:00 创建当日唯一编排任务，先复用或创建增量智能归类任务；归类结束后再复用或创建项目知识任务。编排任务每 10 分钟检查一次状态，不长期占用 Worker handler，也不会对仍在运行的任务创建副本。项目知识 job 有效期为 72 小时，以容纳节流后的跨日分析。
...
采集载荷通过 Schema 校验后、生成快照哈希和搜索文本前，服务端会对密码/密钥赋值、Authorization、常见平台 Token、私钥、数据库连接串、带认证 URL、SSH/SFTP 命令及“主机端口 + 账号/密码”文本执行不可逆入库脱敏。该入口被网页采集、本地同步和历史导入共用，因此 Codex、OpenClaw、Claude Code 也不能绕过。启用的自定义规则同样作用于后续入库。

发送给模型的数据会再次经过脱敏，并额外处理手机号、邮箱、身份证、银行卡、IP 和域名。会话材料默认只包含用户与 AI 正文，不包含 `reasoning` 和 `tool_status` 分段。设置页可预览规则结果、一键启用安全规则包，并通过 Worker 后台任务清理已有消息正文、搜索索引、知识、报告与日志中的匹配内容。
...
| 更新记录 | `/changelog` | 构建时载入 `docs/CHANGELOG.md`，按时间倒序展示各历史版本；仅从侧栏底部小入口进入。 |
...
flowchart LR
  ...
  App --> Monitor["host-monitor 容器 9091，仅内部网络"]
