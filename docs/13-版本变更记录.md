...
- 修复分页项目统计关联列未限定导致会话数显示为 0 的回归；展开项目后自动分页加载会话，时间线补齐项目卡片的水平留白与稳定日期列。CSV、Markdown、XLSX 导出增加生成中与失败反馈，其中 CSV/Markdown 改为流式响应；项目上下文改为批量读取，基础历史索引不再加载无用正文。
...
- 组件下载的枚举参数与 UUID 路由校验解耦，`chrome/windows/macos` 不再误报 Invalid UUID；UUID 动态接口增加全量回归测试。
- Adapter 历史记录缺少版本时统一显示“版本未上报”；活动列表改为固定列轨道、会话多选框固定尺寸；总览“需要处理”五项在桌面端保持一行。
- 自动归类不再把全部项目无差别送入模型：基于会话材料筛选最多 80 个正相关候选，零相关建议降级，错误旧 assignment 不再稳定缓存；已修正误入“生活消费与饮食出行”的 Qwen 服务器配置会话。
- 标签身份增加汉字/拉丁字母或数字边界空格归一；Migration 0022 合并 8 组生产存量视觉重复标签并保留较高置信度、manual 来源与 lock。
- 服务端、Web、Chrome、本地同步代理与共享包统一为 `V2.3.0`；新增 migration 0019–0022。
...
- PostgreSQL 17 空库实际执行 Migration 0000–0022；V2.2 基线 0000–0018 写入会话、Revision、消息、项目、标签和设置夹具后升级到 0022，关系、历史不可验证标记和重复标签关系合并均符合设计。
