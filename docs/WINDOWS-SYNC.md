...
2. 解压 `ai-conversation-archive-windows-sync-V2.1.0.zip` 到任意目录，例如 `C:\AIArchiveSync`。
...
计划任务名与配置目录继续使用旧英文兼容标识，界面与终端品牌已更新为“知言归藏”。`V2.0.1` 会在扫描期间收到文件变化时安排后续扫描，修正 Codex 文件 mtime 不变时的修订时间，并兼容 JSONL 记录内部的独立回车空白；升级后建议执行一次 `rebuild-only` 补齐近期会话，再重新安装后台任务。
