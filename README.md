...
当前发布版本：服务端与 Web `V260822-6`；Chrome 插件与本地同步代理 `V260822-4`。
...
| 包 | 路径 |
| --- | --- |
| NAS 服务端源码包 | `release/ai-conversation-archive-nas-V260822-6-clean-install.tar.gz` |
...
| 模块 | 路径 | 职责 |
| --- | --- | --- |
| 服务端 | `apps/server` | Fastify API、认证、采集入库、Drizzle/PostgreSQL、pg-boss Worker、分析、脱敏、备份、邮件、日志和只读项目容器监测进程。 |
